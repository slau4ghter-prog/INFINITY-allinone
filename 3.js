...
const TARGET_MESSAGES = [
 ...
 "TERA BAAP INFINITY ⃝𖤐 KE ALAWA KON HAI",
  "ʙʜᴀɢᴏᴅᴇ 🢣🍆",
  "ɴᴀᴋʟɪ sᴘᴀᴍᴍᴇʀ 🢣🍆"
];
...
function generateMenuImage({ PREFIX, MODE, GLOBAL_DELAY }) {
  ...
  ctx.font = "24px Arial";
  ctx.fillText("Powered by INFINITY BOT", width / 2, height - 35);
...
if (isCommand(text, "menupic")) {
  ...
  await sock.sendMessage(
    ...
    {
      image: img,
      caption: "📸 INFINITY BOT — Full Command Menu"
    },
    { quoted: msg }
  );
...
await sock.sendMessage(
  ...
  if (text === "2") {
    ...
    await sock.sendMessage(
      ...
      {
        image: fullMenuImg,
        caption: "📸 *INFINITY BOT — ALL COMMANDS*"
      },
      { quoted: msg }
    );
...
if (isCommand(text, "stats")) {
  ...
  const caption =
    `📊 *_INFINITY BOT V07 SYSTEM STATS_*\n\n` +
